...
class ATLASDataset(Dataset):
    def __init__(
        ...
        print(f"Loading ATLAS dataset from {filepath} with {num_events} samples")
    ...
    def load_data(self, tree):
        ...
        if self.remove_wrong_idxs:
            ...
            track_particle_idxs = arrays["track_particle_idx"]
    ...
    def load_event(self, idx):
        ...
        topo_pt = self.full_data_array["topo_pt"][topo_start:topo_end]
        ...
        topo_num_cells = self.full_data_array["topo_num_cells"][topo_start:topo_end]
        topo_em_frac = self.full_data_array["topo_em_frac"][topo_start:topo_end]
        topo_center_mag = self.full_data_array["topo_center_mag"][topo_start:topo_end]
        topo_center_lambda = self.full_data_array["topo_center_lambda"][topo_start:topo_end]
        ...
        node_features = {
            ...
            "pt": torch.cat(
                [
                    ...
                    self.scaler.transforms["pt"].transform(topo_pt),
                ],
                -1,
            ),
            "eta": torch.cat(
            ...
            "num_cells": torch.cat(
                [
                    torch.zeros(n_tracks, dtype=torch.float32),
                    self.scaler.transforms["num_cells"].transform(topo_num_cells),
                ],
                -1,
            ),
            "em_frac": torch.cat(
                [
                    torch.zeros(n_tracks, dtype=torch.float32),
                    self.scaler.transforms["em_frac"].transform(topo_em_frac),
                ],
                -1,
            ),
            "center_mag": torch.cat(
                [
                    torch.zeros(n_tracks, dtype=torch.float32),
                    self.scaler.transforms["center_mag"].transform(topo_center_mag),
                ],
                -1,
            ),
            "center_lambda": torch.cat(
                [
                    torch.zeros(n_tracks, dtype=torch.float32),
                    self.scaler.transforms["center_lambda"].transform(topo_center_lambda),
                ],
                -1,
            ),
            # flags
        ...
        node_raw_features = {
            ...
            "raw_pt": torch.cat(
                [
                    track_pt,
                    torch.zeros(n_topos, dtype=torch.float32),
                    # we have the topo pTs, but not adding them here
                    # because idk if some code assumes they are zero
                ],
                -1,
            ),
...
class PflowDataModule(L.LightningDataModule):
    def __init__(
        ...
        num_val: int,
        num_val_per_epoch: int | None,
        num_test: int,
        num_test_per_epoch: int | None,
        scale_dict_path: str,
        ...
        **kwargs,
        ...
        self.num_val_per_epoch = num_val_per_epoch
        self.num_test = num_test
        self.num_test_per_epoch = num_test_per_epoch
    ...
    def get_dataloader(self, stage: str, dataset: ATLASDataset, shuffle: bool, num_per_epoch: int | None = None):
        print(f"Creating {stage} dataloader with {len(dataset):,} events")

        sampler = None

        if num_per_epoch is not None:
            if num_per_epoch > len(dataset):
                raise ValueError(f"num_per_epochs={num_per_epoch} exceeds dataset size {len(dataset)}")
    ...
    def train_dataloader(self):
        ...
        return self.get_dataloader(dataset=self.train_dset, stage="fit", shuffle=True, num_per_epoch=self.num_train_per_epoch)

    def val_dataloader(self):
        print("Instantiating validation dataloader on rank", self.trainer.local_rank)
        return self.get_dataloader(dataset=self.val_dset, stage="test", shuffle=False, num_per_epoch=self.num_val_per_epoch)

    def test_dataloader(self):
        print("Instantiating test dataloader on rank", self.trainer.local_rank)
        return self.get_dataloader(dataset=self.test_dset, stage="test", shuffle=False, num_per_epoch=self.num_test_per_epoch)
