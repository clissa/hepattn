...
FIG_W = 12; FIG_H_1ROW = 4.5; FIG_DPI = 300
...
def plot_scatter(ref_dict, comp_dict, comp_name, ref_name='truth'):
    ...
    for v_i, var in enumerate(['pt', 'eta', 'phi']):
        for cl_i, (cl_name, mask_dict) in enumerate(_cl_masks.items()):
            ...
            ax.hist2d(ref_dict[var][mask_dict['ref']], comp_dict[var][mask_dict['comp']], 
                bins=50, range=((_min,_max), (_min,_max)), cmap=cmap, norm=LogNorm())
            ax.minorticks_on()
...
def plot_residuals(data_dicts, ref_name='truth', pt_relative=False, log_y=False, qs=None, stylesheet=None):
    ...
    _COLORS, _LABELS, _HISTTYPES, _ALPHAS, _LINE_STYLES, _LABEL_LEN, _ = update_stylesheet(stylesheet)

    residual_dict = {}
    for name in data_dicts.keys():
        residual_dict[name] = {'pt': [], 'eta': [], 'phi': [], 'ref_class': []}
    abs_max_dict = {
        'Charged': {'pt': 0, 'eta': 0, 'phi': 0}, 'Neutral': {'pt': 0, 'eta': 0, 'phi': 0}}
    if qs is None:
        qs = {
            'Charged': {'pt': 97, 'eta': 97, 'phi': 97}, 'Neutral': {'pt': 97, 'eta': 97, 'phi': 97}}
    ...
    for name, (ref_dict, comp_dict, _, _) in data_dicts.items():
        for var in ['pt', 'eta', 'phi']:
            ...
            abs_max_dict['Charged'][var] = max(abs_max_dict['Charged'][var], 
                np.percentile(np.abs(residual_dict[name][var][mask]), qs['Charged'][var]))
            abs_max_dict['Neutral'][var] = max(abs_max_dict['Neutral'][var], 
                np.percentile(np.abs(residual_dict[name][var][~mask]), qs['Neutral'][var]))
    ...
    for name, res_dict in residual_dict.items():
        ...
        cl_masks = {
            'Charged': res_dict['ref_class'] <= 2, 'Neutral': res_dict['ref_class'] >= 3}
        ...
        for cl_i, (cl_name, cl_mask) in enumerate(cl_masks.items()):
            for v_i, var in enumerate(['pt', 'eta', 'phi']):
                ...
                custom_hist_v2(ax, res_dict[var][cl_mask], label_length=-1, metrics="mean std iqr", bins=bins,
...
def plot_residuals_neutrals(data_dicts, ref_name='truth', pt_relative=False, log_y=False, qs=None, stylesheet=None, separate_figures=False):
    ...
    abs_max_dict = {
        'Neutral hadron': {'pt': 0, 'eta': 0, 'phi': 0}, 'Photon': {'pt': 0, 'eta': 0, 'phi': 0}}
    if qs is None:
        qs = {
            'Neutral hadron': {'pt': 97, 'eta': 97, 'phi': 97}, 'Photon': {'pt': 97, 'eta': 97, 'phi': 97}}
    ...
    for name, (ref_dict, comp_dict, ref_dict_unmatched, _) in data_dicts.items():
        for var in ['pt', 'eta', 'phi']:
            ...
            abs_max_dict['Neutral hadron'][var] = max(abs_max_dict['Neutral hadron'][var], 
                np.percentile(np.abs(residual_dict[name][var][mask]), qs['Neutral hadron'][var]))
            
            mask = ref_dict['class'] == 4
            abs_max_dict['Photon'][var] = max(abs_max_dict['Photon'][var], 
                np.percentile(np.abs(residual_dict[name][var][~mask]), qs['Photon'][var]))
    ...
    if separate_figures:
        ...
        for i in range(n_row):
            ...
            for j in range(n_col):
                ...
                figs.append(fig); tmp_axs.append(ax)
            axs.append(tmp_axs)
    else:
    ...
    for name, res_dict in residual_dict.items():
        cl_masks = {
            'Neutral hadron': res_dict['ref_class'] == 3, 'Photon': res_dict['ref_class'] == 4}

        for cl_i, (cl_name, cl_mask) in enumerate(cl_masks.items()):
            for v_i, var in enumerate(['pt', 'eta', 'phi']):
                ax = axs[cl_i][v_i]
                bins = np.linspace(-abs_max_dict[cl_name][var], abs_max_dict[cl_name][var], 50)
                custom_hist_v2(ax, res_dict[var][cl_mask], label_length=-1, metrics="mean std iqr", f=res_dict['f'][cl_name], bins=bins,
                    histtype=_HISTTYPES[name], label=_LABELS[name], color=_COLORS[name], linestyle=_LINE_STYLES[name], alpha=_ALPHAS[name])
    ...
    for axs_row in axs:
        for ax in axs_row:
            ax.legend(fontsize=8) # loc='lower left', bbox_to_anchor=(-0.27, 1.01))
            ax.set_ylim(0, ax.get_ylim()[1] * (1+len(residual_dict)*0.23))
...
def plot_eff_fr_purity(eff_fr_purity_input_dict, stylesheet=None):
    ...
    for name, hung_part_dict in eff_fr_purity_input_dict.items():
        ...
        n_bins  = 10
        pt_bins = np.logspace(np.log10(1), np.log10(500), n_bins + 1)
        pt_bin_mids = (pt_bins[1:] + pt_bins[:-1]) / 2

        eff_fr_dict = {
            'neut had': {'class': 3, 'color': 'cornflowerblue'},
            'photon': {'class': 4, 'color': 'orange'}
        }
        ...
        for cl_name, cl_dict in eff_fr_dict.items():
            for bin in range(len(pt_bins) - 1):
                truth_mask_matched   = (truth_pt_matched   > pt_bins[bin]) & (truth_pt_matched   <= pt_bins[bin+1]) & (truth_class_matched == cl_dict['class'])
                truth_mask_unmatched = (truth_pt_unmatched > pt_bins[bin]) & (truth_pt_unmatched <= pt_bins[bin+1]) & (truth_class_unmatched == cl_dict['class'])
                pflow_mask_matched   = (pflow_pt_matched   > pt_bins[bin]) & (pflow_pt_matched   <= pt_bins[bin+1]) & (pflow_class_matched == cl_dict['class'])
                pflow_mask_unmatched = (pflow_pt_unmatched > pt_bins[bin]) & (pflow_pt_unmatched <= pt_bins[bin+1]) & (pflow_class_unmatched == cl_dict['class'])
        ...
        for cl_name, cl_dict in eff_fr_dict.items():
            ax1.hist(pt_bin_mids, bins=pt_bins, weights=cl_dict['eff'], ls=_LINE_STYLES[name],
            label=f'{cl_name} ({_LABELS[name]})', histtype='step', linewidth=1, color=_COLORS[name][cl_name])

        for cl_name, cl_dict in eff_fr_dict.items():
            ax2.hist(pt_bin_mids, bins=pt_bins, weights=cl_dict['fr'], ls=_LINE_STYLES[name],
            label=f'{cl_name} ({_LABELS[name]})', histtype='step', linewidth=1, color=_COLORS[name][cl_name])

        for cl_name, cl_dict in eff_fr_dict.items():
            ax3.hist(pt_bin_mids, bins=pt_bins, weights=cl_dict['purity'], ls=_LINE_STYLES[name],
            label=f'{cl_name} ({_LABELS[name]})', histtype='step', linewidth=1, color=_COLORS[name][cl_name])
