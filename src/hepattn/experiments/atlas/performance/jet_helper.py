...
class JetHelper:
    ...
    def getPtSortedCheapJets(self, four_vectors, pt_min=8, n_const_min=2, eta_max=2.5, store_constituent_idxs=False):
        return [CheapJet(j, n_const=nc, constituent_idxs=const_idxs) 
            for (j, nc, const_idxs) in self.getPtSortedJets(
                four_vectors, pt_min, n_const_min, eta_max, get_constituent_idxs=store_constituent_idxs)]
    ...
    def compute_jets(self, pts, etas, phis, fourths, fourth_name, store_constituent_idxs=False):
        ...
        for ev_i in range(n_events):
            four_vec = self.GetVectors(
                pts[ev_i], etas[ev_i], phis[ev_i], fourths[ev_i], fourth_name
            )
            jets.append(self.getPtSortedCheapJets(four_vec, store_constituent_idxs=store_constituent_idxs))
            if ev_i % 1000 == 0:
                print(f'Processed {ev_i}/{n_events} events...')
        return jets
    ...
    if n_procs > 0:
        with Pool(n_procs) as pool:
            ...
            for i, (start, end) in enumerate(zip(starts, ends)):
                result = pool.apipe(compute_jet_multiproc,
